...
void Client_cbWrited(uv_write_t* req, int status)
{
    ...
    g_cache_write_req_Client.Free((UVWriteReq*)req);
}
...
void Client_cbRead(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf) 
{
    ...
    if (nread < 0)
    {
        if (nread != UV_EOF)cout<<"[ThreadClient] Read error :"<<uv_err_name(nread)<<endl;
